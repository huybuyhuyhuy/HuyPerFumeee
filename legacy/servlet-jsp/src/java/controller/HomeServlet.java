...
public class HomeServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        // 2. BANNER ẢNH UNSPLASH (cache 24h)
        try {
            request.setAttribute("bannerSlide1", UnsplashImageService.getImage("luxury perfume bottle golden"));
            request.setAttribute("bannerSlide2", UnsplashImageService.getImage("elegant fragrance collection"));
            request.setAttribute("bannerSlide3", UnsplashImageService.getImage("premium perfume essence luxury"));
            request.setAttribute("bannerSlide4", UnsplashImageService.getImage("exclusive niche perfume aroma"));
            request.setAttribute("bannerMen", UnsplashImageService.getImage("men cologne luxury perfume"));
            request.setAttribute("bannerWomen", UnsplashImageService.getImage("women luxury floral perfume"));
            request.setAttribute("bannerUnisex", UnsplashImageService.getImage("unisex fragrance luxury perfume"));
        } catch (Exception e) {
            getServletContext().log("Error loading Unsplash banners", e);
        }

        // 3. NHẬN THAM SỐ PHÂN TRANG & LỌC
        int pageSize = 12; 
        int currentPage = 1;
