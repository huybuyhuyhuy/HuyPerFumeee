...
<body>
    ...
    <div class="success-container">
        ...
        <div class="order-content text-center">
            ...
            <p class="order-id">MÃ ĐƠN HÀNG: #HP-${not empty orderId ? orderId : (System.currentTimeMillis() % 1000000)}</p>
            ...
            <div class="receipt-card text-start">
                <div class="receipt-item">
                    ...
                    <span class="receipt-value">
                        <c:choose>
                            ...
                            <c:when test="${paymentMethod == 'ZaloPay'}">Ví ZaloPay</c:when>
                ...
                <c:if test="${not empty receiptItems}">
                    <hr>
                    <div class="small text-muted mb-2 fw-bold">CHI TIẾT HÓA ĐƠN KỸ THUẬT SỐ</div>
                    <c:forEach items="${receiptItems}" var="it">
                        <div class="receipt-item">
                            <span class="receipt-label">${it.name}</span>
                            <span class="receipt-value">SKU: ${empty it.sku ? 'N/A' : it.sku}</span>
                        </div>
                        <div class="receipt-item">
                            <span class="receipt-label">Batch code:</span>
                            <span class="receipt-value">${empty it.batch_code ? 'N/A' : it.batch_code}</span>
                        </div>
                    </c:forEach>
                </c:if>
                <div class="total-row">
                    ...
                    <span class="float-end">
                        <c:choose>
                            <c:when test="${not empty orderTotal}">
                                <fmt:formatNumber value="${orderTotal}" pattern="#,##0"/>đ
                            </c:when>
            ...
            <div class="footer-note">
                <i class="fas fa-shield-alt me-1"></i> Cam kết chính hãng - SKU và Batch Code trên hóa đơn là tem chống hàng giả kỹ thuật số.
            </div>
        </div>
    </div>
