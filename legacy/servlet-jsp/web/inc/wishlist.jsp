                        ...
                        <div class="card wishlist-card shadow-sm h-100">
                            ...
                            <c:choose>
                                <c:when test="${not empty p.image && (p.image.startsWith('http') || p.image.endsWith('.jpg') || p.image.endsWith('.jpeg') || p.image.endsWith('.png') || p.image.endsWith('.webp') || p.image.endsWith('.gif'))}">
                                    <img src="${p.image.startsWith('http') ? p.image : pageContext.request.contextPath.concat('/api/image?name=').concat(p.image)}" class="card-img-top wishlist-img" alt="${p.name}">
                                </c:when>
                                <c:otherwise>
                                    <img src="https://loremflickr.com/400/400/perfume,bottle,${p.name}/all?lock=${p.id}" class="card-img-top wishlist-img" alt="${p.name}">
                                </c:otherwise>
                            </c:choose>
