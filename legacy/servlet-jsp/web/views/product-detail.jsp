                ...
                <div class="product-gallery">
                    <c:choose>
                        <c:when test="${not empty product.image && (product.image.startsWith('http') || product.image.endsWith('.jpg') || product.image.endsWith('.jpeg') || product.image.endsWith('.png') || product.image.endsWith('.webp') || product.image.endsWith('.gif'))}">
                            <img src="${product.image.startsWith('http') ? product.image : pageContext.request.contextPath.concat('/api/image?name=').concat(product.image)}" class="product-main-img" alt="${product.name}">
                        </c:when>
                        <c:otherwise>
                        ...
                        <div class="product-info-section">
                            ...
                            <div class="scent-notes">
                                ...
                                <div class="note-item">
                                    ...
                                    <span class="note-value">${scentStructure[0]}</span>
                                </div>
                                <div class="note-item">
                                    <span class="note-label">Hương giữa:</span>
                                    <span class="note-value">${scentStructure[1]}</span>
                                </div>
                                <div class="note-item">
                                    <span class="note-label">Hương cuối:</span>
                                    <span class="note-value">${scentStructure[2]}</span>
                                </div>
                            </div>
                ...
                <div class="product-tabs">
                    ...
                    <div class="tab-content" id="myTabContent">
                        ...
                        <div class="tab-pane fade" id="storage" role="tabpanel">
                            <ul>
                                <li><strong>Nhiệt độ lý tưởng:</strong> 18-25°C, tránh thay đổi nhiệt độ đột ngột để giữ độ ổn định tầng hương.</li>
                                <li><strong>Ánh sáng:</strong> Luôn để chai ở nơi râm mát, không đặt sát cửa sổ hoặc cốp xe có nhiệt cao.</li>
                                <li><strong>Cách đậy nắp:</strong> Đóng nắp ngay sau khi dùng để hạn chế bay hơi cồn và oxy hóa tinh dầu.</li>
                                <li><strong>Vị trí bảo quản:</strong> Fullbox nên để trong hộp gốc, decant nên dựng đứng trong khay chống đổ.</li>
                                <li><strong>Tuổi thọ mùi:</strong> Nên dùng tốt nhất trong 24-36 tháng sau khi mở nắp để đạt hiệu năng chuẩn.</li>
                            </ul>
                        </div>
                        <div class="tab-pane fade" id="policy" role="tabpanel">
                            <ul>
                                <li><strong>Thời hạn hỗ trợ:</strong> Đổi trả trong vòng 03 ngày từ khi nhận hàng theo mã vận đơn.</li>
                                <li><strong>Điều kiện áp dụng:</strong> Sản phẩm lỗi do nhà sản xuất, rò rỉ vòi xịt, hoặc giao sai SKU/Batch đã đặt.</li>
                                <li><strong>Không áp dụng:</strong> Sản phẩm đã dùng quá 10%, trầy xước do người dùng hoặc không còn tem niêm phong.</li>
                                <li><strong>Hồ sơ cần có:</strong> Video unbox + ảnh tem đáy chai + mã đơn hàng để đối soát trong 24h.</li>
                                <li><strong>Hoàn tiền:</strong> Đơn thanh toán MoMo đủ điều kiện sẽ được hoàn tự động và cập nhật trạng thái `Refunded`.</li>
                            </ul>
                        </div>
                    </div>
                </div>
