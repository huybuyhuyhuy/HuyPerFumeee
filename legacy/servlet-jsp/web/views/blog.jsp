...
<style>
    .blog-wrap { background: #f4f7f6; padding: 50px 0; }
    ...
    .post-row { display: flex; gap: 24px; border-bottom: 1px solid #eee; padding-bottom: 24px; margin-bottom: 24px; transition: .3s; }
    .post-row:last-child { border-bottom: none; margin-bottom: 0; }
    .post-img-link { width: 300px; height: 200px; flex-shrink: 0; border-radius: 12px; overflow: hidden; display: block; }
    .post-row img { width: 100%; height: 100%; object-fit: cover; transition: 0.5s; }
    .post-row:hover img { transform: scale(1.08); }
    .post-body { flex: 1; }
    ...
    .post-title a { color: #222; text-decoration: none; transition: 0.2s; }
    .post-title a:hover { color: #003D2E; }
    .post-excerpt { color: #666; font-size: 14px; line-height: 1.6; margin-bottom: 15px; display: -webkit-box; -webkit-line-clamp: 3; -webkit-box-orient: vertical; overflow: hidden; }
            ...
            <div class="col-lg-3">
                <div class="side-box">
                    ...
                    <ul class="nav-list">
                        ...
                        <li><a href="knowledge">Kiến thức</a></li>
                ...
                <div class="content-box">
                    <div class="d-flex justify-content-between align-items-center mb-4">
                        ...
                        <div class="d-flex gap-2">
                            <input id="blog-search" class="form-control form-control-sm" style="width: 200px;" placeholder="Tìm kiếm bài viết...">
                            <select id="blog-tag" class="form-select form-select-sm" style="width: 150px;">
                    ...
                    <div id="blog-pagination" class="d-flex justify-content-center gap-2 mt-4"></div>
                </div>
            </div>
        </div>
    </div>
...
<script>
    ...
    async function loadBlog() {
        const url = '${pageContext.request.contextPath}/api/content?type=blog&page=' + blogPage + '&pageSize=4'
        ...
        function render(items) {
            list.innerHTML = items.map(function(item) {
                const fallback = 'https://parfumerie.vn/storage/bai-viet/phieu-khao-sat-y-kien-khach-hang-2.jpg';
                return '<article class="post-row">'
                    + '<a href="${pageContext.request.contextPath}/blog-detail?id=' + item.id + '" class="post-img-link">'
                    ...
                    + '<h4 class="post-title"><a href="${pageContext.request.contextPath}/blog-detail?id=' + item.id + '">' + item.title + '</a></h4>'
                    + '<p class="post-excerpt">' + item.excerpt + '</p>'
                    + '<a href="${pageContext.request.contextPath}/blog-detail?id=' + item.id + '" class="btn btn-sm btn-outline-dark rounded-pill px-3">Đọc tiếp</a>'
                    + '</div></article>';
            }).join('');
            if (!items.length) {
        ...
        highlights.innerHTML = (data.highlights || []).map(function(item) {
            return '<a href="${pageContext.request.contextPath}/blog-detail?id=' + item.id + '" class="text-decoration-none highlight-item">'
