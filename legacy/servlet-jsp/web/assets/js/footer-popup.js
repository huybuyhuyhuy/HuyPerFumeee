...
function showRandomSale() {
    ...
    if (imgElement) {
        if (randomProduct.image && randomProduct.image.startsWith('http')) {
            imgElement.src = randomProduct.image;
        } else if (randomProduct.image && /\.(jpg|jpeg|png|webp|gif)$/i.test(randomProduct.image)) {
            imgElement.src = basePath + '/api/image?name=' + encodeURIComponent(randomProduct.image);
        } else {
            imgElement.src = basePath + '/api/image?name=perfume-placeholder.png';
        }
        imgElement.onerror = function() {
            this.onerror = null;
            this.src = basePath + '/api/image?name=perfume-placeholder.png';
        };
    }
