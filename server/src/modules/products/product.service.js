...
async function mapProductDetail(row, { userId = null } = {}) {
  const [variantRows, imageRows, isFavorite] = await Promise.all([
    findProductVariants(row.id),
    findProductImages(row.id),
    isFavoriteProduct(userId, row.id),
  ]);

  return attachValidation(mapProductRow(row, {
...
export async function getProductById(id, options = {}) {
  ...
  const cached = await getCache(cacheKey);
  if (cached) {
    const product = cloneProduct(cached);
    if (options.userId) {
...
export async function getProductPurchaseOption(productId, variantId = null) {
  const product = await getProductById(productId);
  return resolvePurchasableSelection(product, variantId);
}
