...
function supportsCartItemType(capabilities) {
  return hasColumn(capabilities.cartItemColumns, 'item_type');
}

function supportsCartItemSelectedVolume(capabilities) {
  return hasColumn(capabilities.cartItemColumns, 'selected_volume_ml');
}
...
function buildCartItemSelect(capabilities) {
  ...
  const itemTypeSelect = supportsCartItemType(capabilities) ? 'ci.item_type' : "N'FULL_BOTTLE' AS item_type";
  const selectedVolumeSelect = supportsCartItemSelectedVolume(capabilities) ? 'ci.selected_volume_ml' : 'NULL AS selected_volume_ml';
  const availableVolumeSelect = supportsCartItemSelectedVolume(capabilities) && capabilities.hasProductBatches
    ? `(SELECT COALESCE(SUM(pb.remaining_volume_ml), 0)
        FROM product_batches pb
        WHERE pb.product_id = ci.product_id AND pb.status = N'ACTIVE') AS available_volume_ml`
    : '0 AS available_volume_ml';
  ...
  return `
    SELECT
      ...
      ${itemTypeSelect},
      ${selectedVolumeSelect},
      ci.quantity,
      ${unitPriceSelect},
      ${availableVolumeSelect},
...
function normalizeCartRow(row) {
  const itemType = String(row.item_type || 'FULL_BOTTLE').toUpperCase() === 'DECANT' ? 'DECANT' : 'FULL_BOTTLE';
  const hasVariant = itemType !== 'DECANT' && Boolean(row.variant_id);
  ...
  const price = itemType === 'DECANT'
    ? positiveNumber(row.unit_price) ?? 0
    : resolvePrice(originalPrice, salePrice) ?? positiveNumber(row.unit_price) ?? 0;
  const selectedVolumeMl = row.selected_volume_ml ? Number(row.selected_volume_ml) : null;
  const decantUnits = itemType === 'DECANT' && selectedVolumeMl ? Math.floor(nonNegativeInt(row.available_volume_ml) / selectedVolumeMl) : stockQuantity;
  ...
  return {
    ...
    product: {
      id: row.product_id,
      variantId: itemType === 'DECANT' ? null : row.product_variant_id || null,
      itemType,
      selectedVolumeMl,
      name: row.product_name || '',
      image: row.variant_image || row.product_image || '',
      price: itemType === 'DECANT' ? price : originalPrice,
      salePrice: itemType === 'DECANT' ? null : salePrice && originalPrice && salePrice < originalPrice ? salePrice : null,
      discountPrice: itemType === 'DECANT' ? null : salePrice && originalPrice && salePrice < originalPrice ? salePrice : null,
      originalPrice: itemType === 'DECANT' ? price : originalPrice,
      stock: itemType === 'DECANT' ? decantUnits : stockQuantity,
      stockQuantity: itemType === 'DECANT' ? decantUnits : stockQuantity,
      availableVolumeMl: nonNegativeInt(row.available_volume_ml),
...
async function backfillLegacyVariantSelections(cartId, capabilities) {
  ...
  if (hasColumn(capabilities.variantColumns, 'variant_type')) {
    variantPredicates.push("UPPER(ISNULL(pv.variant_type, 'FULL')) <> 'DECANT'");
  }
  ...
  const unitPriceAssignment = supportsCartItemUnitPrice(capabilities)
    ...
    : '';
  const decantSkipClause = supportsCartItemType(capabilities)
    ? "AND ISNULL(ci.item_type, N'FULL_BOTTLE') <> N'DECANT'"
    : '';
...
export async function upsertDurableCartItem(scope, selection, quantity, mode = 'add') {
  ...
  if (selection.itemType === 'DECANT' && (!supportsCartItemType(capabilities) || !supportsCartItemSelectedVolume(capabilities))) {
    return { code: 500, message: 'Schema cart_items chua ho tro item_type va selected_volume_ml' };
  }
  ...
  try {
    ...
    const itemType = String(selection.itemType || 'FULL_BOTTLE').toUpperCase() === 'DECANT' ? 'DECANT' : 'FULL_BOTTLE';
    const selectedVolumeMl = selection.volumeMl ? Number(selection.volumeMl) : null;
    ...
    if (supportsCartItemType(capabilities)) existingRequest.input('itemType', sql.NVarChar, itemType);
    if (supportsCartItemSelectedVolume(capabilities)) existingRequest.input('selectedVolumeMl', sql.Int, selectedVolumeMl);
    const variantWhere = supportsCartItemVariant(capabilities)
      ? 'AND ISNULL(product_variant_id, 0) = ISNULL(@variantId, 0)'
      : '';
    const itemTypeWhere = supportsCartItemType(capabilities)
      ? 'AND ISNULL(item_type, N\'FULL_BOTTLE\') = @itemType'
      : '';
    const selectedVolumeWhere = supportsCartItemSelectedVolume(capabilities)
      ? 'AND ISNULL(selected_volume_ml, 0) = ISNULL(@selectedVolumeMl, 0)'
      : '';
    const existingResult = await existingRequest.query(
      `SELECT TOP 1 id, quantity
       ...
       WHERE cart_id = @cartId
         ...
         ${variantWhere}`
         + ` ${itemTypeWhere} ${selectedVolumeWhere}`
    );
    ...
    if (existing) {
      ...
      if (supportsCartItemType(capabilities)) {
        insertRequest.input('itemType', sql.NVarChar, itemType);
        insertColumns.push('item_type');
        insertValues.push('@itemType');
      }
      if (supportsCartItemSelectedVolume(capabilities)) {
        insertRequest.input('selectedVolumeMl', sql.Int, selectedVolumeMl);
        insertColumns.push('selected_volume_ml');
        insertValues.push('@selectedVolumeMl');
      }
...
export async function removeDurableCartItem(scope, productId, variantId = null, options = {}) {
  const capabilities = await getCheckoutStorageCapabilities();
  if (!capabilities.hasDurableCart) return null;
  if (scope.type === 'guest' && !supportsGuestCart(capabilities)) return null;
  ...
  if (supportsCartItemType(capabilities) && options.itemType) {
    variantSql += " AND ISNULL(item_type, N'FULL_BOTTLE') = ?";
    params.push(String(options.itemType || 'FULL_BOTTLE').toUpperCase() === 'DECANT' ? 'DECANT' : 'FULL_BOTTLE');
  }
  if (supportsCartItemSelectedVolume(capabilities) && options.volumeMl) {
    variantSql += ' AND ISNULL(selected_volume_ml, 0) = ISNULL(?, 0)';
    params.push(Number(options.volumeMl));
  }
