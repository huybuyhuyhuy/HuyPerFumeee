...
export function mapProductRow(row, { variants = [], imageRows = [], isFavorite = false, includeVariants = false } = {}) {
  ...
  const discoveryOriginalPrice = row.price;
  const discoverySalePrice = row.sale_price ?? row.discount_price;
  ...
  return {
    ...
    category: row.is_decant || row.has_decant_variant ? { id: 5, name: 'Decant' } : (row.category_id ? { id: row.category_id, name: normalizeText(row.category_name) } : null),
    ...
    bottleVolumeMl: toNonNegativeInt(row.bottle_volume_ml) || toNonNegativeInt(row.volume_ml) || 100,
    isOpenedBottle: toNonNegativeInt(row.opened_ml) > 0,
    createdAt,
    updatedAt,
    deletedAt,
  };
