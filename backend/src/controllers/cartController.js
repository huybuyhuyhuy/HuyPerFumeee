...
export async function addCart(req, res) {
  try {
    ...
    const options = {
      ...
      volumeMl: req.body?.volumeMl ?? req.body?.selectedVolumeMl,
    };
    if (!productId) {
      return errorResponse(res, 400, 'productId không hợp lệ');
    }

    const result = await addToCart(scope, productId, quantity, variantId, options);
    if (result.code) {
...
export async function updateCart(req, res) {
  try {
    ...
    const result = await updateCartItem(scope, productId, quantity, variantId, options);
    if (result.code) {
      return errorResponse(res, result.code, result.message);
    }
...
export async function removeCart(req, res) {
  try {
    ...
    const options = {
      itemType: req.query?.itemType ?? req.body?.itemType,
      volumeMl: req.query?.volumeMl ?? req.query?.selectedVolumeMl ?? req.body?.volumeMl ?? req.body?.selectedVolumeMl,
    };
    const result = await removeCartItem(scope, productId, variantId, options);
